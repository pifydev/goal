...
- **Sisyphus mode** (v0.4): write the objective as a list — one step per line, or separated by `;` — and the goal is worked in order. Every continuation prompt names the current step and nothing else, `goal_step_done(evidence)` advances the cursor one step forward (never sideways), and `goal_complete` is refused while steps remain: an ordered goal is finished when its list is, not when the agent feels done with the interesting part. Prose objectives are unaffected — a sentence with commas is not a plan, so it is never parsed as one.
